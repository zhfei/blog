...
let createTopicTable = `CREATE TABLE IF NOT EXISTS \`topic\`(
   ...
   \`replies\` VARCHAR(255) NULL COMMENT '回复对象id数组',
   \`created_time\` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
let createReplayTable = `CREATE TABLE IF NOT EXISTS \`replay\`(
   \`id\` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '自增id',
   \`reply_email\` VARCHAR(100) NOT NULL COMMENT '邮箱地址',
   \`reply_content\` VARCHAR(255)  COMMENT '文章内容',
   \`reply_time\` TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
   PRIMARY KEY ( \`id\` )
